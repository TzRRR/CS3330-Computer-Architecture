...
int global_array[2048568] __attribute__((aligned(128)));
...
int main() {
    const int MAX = 2048568;
    const int SKIP = 4;
    const int ITERS = 240000;
...
#pragma clang loop interleave(disable)
    ...
    for (int i = 0; i < ITERS; ++i) {
      for (int k = 0; k < 128; k++){
	j = j + global_array[k*16 + 16*2* 256];
	j = j + global_array[k*16 + 16*256];
	j = j + global_array[k*16 + 16*3*256];
      }
    }
