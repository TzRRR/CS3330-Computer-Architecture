...
def count_time_in(args, fh):
    ...
    for instruction in csv_reader:
        ...
        if last_instruction != None:
            ...
            if args.load_use_hazard and forward_from_last and last_instruction['is_memory_read'] == 'Y':
                load_use_delay += 1
	    if forward_from_last == True and args.items == 5:
		branch_delay = branch_delay + 1
	...
	if instruction['is_conditional_branch'] == 'Y' and instruction['branch_taken'] == 'N' and args.items != 3 and args.items != 4 and args.items != 5 and args.items != 6:
		branch_delay += args.branch_delay
        if instruction['is_conditional_branch'] == 'Y' and instruction['branch_taken'] == 'N' and args.items == 3:
		branch_delay = branch_delay + args.branch_delay + 1
	if instruction['is_conditional_branch'] == 'Y' and args.items == 4:
		branch_delay = branch_delay + 2

	if instruction['is_conditional_branch'] == 'Y'and args.items == 6:
		if pred_dict.has_key(instruction['orig_pc']) == False:
			pred_dict[instruction['orig_pc']] = instruction['branch_taken']
			if instruction['branch_taken'] == 'N':
				branch_delay = branch_delay + 2
		else: 
			if pred_dict[instruction['orig_pc']] != instruction['branch_taken']:
				branch_delay = branch_delay + 2
				pred_dict[instruction['orig_pc']] = instruction['branch_taken']
...
def main():
    ...
    parser.add_argument('items', default = 0, type=int)
